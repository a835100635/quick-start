...
- 首次启动时会在当前屏幕的胶囊旁显示引导气泡，提示触发器位置和操作方式。
- 将鼠标移至屏幕边缘中间的触发器，或点击触发器，展开半圆快捷菜单。
- 点击圆心的齿轮图标打开设置；可在“触发类型”中选择：
  - **鼠标悬浮**：鼠标移入触发器时展开。
  - **鼠标点击**：点击触发器时展开。
- 在设置中可添加、删除或排序应用与文件夹，并调整菜单半径。
- 拖拽屏幕边缘的触发器可上下移动，也可拖到另一侧。
- 按 `⇧ + Esc`，在当前鼠标所在屏幕的中心展开完整圆形菜单。
